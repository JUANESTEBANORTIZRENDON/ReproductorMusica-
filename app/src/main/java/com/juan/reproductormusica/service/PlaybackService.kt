...
class PlaybackService : MediaSessionService() {
    ...
    private fun initializeSessionAndPlayer() {
        ...
        player.addListener(object : Player.Listener {
            ...
            override fun onPlaybackStateChanged(playbackState: Int) {
                // 1) Si termina la playlist, no hay repeat y el shuffle está activo,
                //    saltar a una canción aleatoria distinta de la actual
                if (
                    ...
                    val pool = currentPlaylist.indices.filter { it != currentIdx }
                    val nextIdx = if (pool.isNotEmpty()) pool.random() else currentIdx
                    player.seekToDefaultPosition(nextIdx)
                    player.play()
                }

                // 2) Control de notificaciones (si están deshabilitadas, suprimir)
        ...
        override fun onCustomCommand(
            ...
            when (customCommand.customAction) {
                ...
                CUSTOM_COMMAND_TOGGLE_REPEAT -> {
                    // Solo alternar entre OFF y ONE (repetir canción actual)
    ...
    private fun suppressAllNotifications() {
        try {
            ...
            // 3. Cancelar por rangos “por si acaso”
            for (id in 1..1000) {
                try { notificationManager.cancel(id) } catch (_: Exception) {}
            }

            // 4. Reintentos rápidos
            val handler = android.os.Handler(android.os.Looper.getMainLooper())
            handler.post { try { stopForeground(STOP_FOREGROUND_REMOVE); notificationManager.cancelAll() } catch (_: Exception) {} }
            handler.postDelayed({ try { stopForeground(STOP_FOREGROUND_REMOVE); notificationManager.cancelAll() } catch (_: Exception) {} }, 50)
            handler.postDelayed({ try { stopForeground(STOP_FOREGROUND_REMOVE); notificationManager.cancelAll() } catch (_: Exception) {} }, 200)

        } catch (_: Exception) {
            // Silencio: no romper el servicio
        }
    }
